...
class Verifier
{
     ...
     * @param string $now
     *   The current date in ISO-8601 (YYYY-MM-DD), for csig expiration tests. (optional; default: today.)
     */
     function __construct($public_key_raw, $now = '') {
    ...
    /**
     * Gets a \DateTimeImmutable object modeling "now," possibly set by the object instantiator.
     *
     ...
     */
     public function getNow()
     {
         if (! $this->now instanceof \DateTimeImmutable) {
             ...
             $now_dt = \DateTimeImmutable::createFromFormat('Y-m-d', $this->now);
    ...
    {
        $checksum_list_raw = $this->verifyCsigMessage($csig_signed_checksum_list);
        return $this->verifyTrustedChecksumList($checksum_list_raw, $working_directory);
    }
    ...
    public function verifyCsigChecksumFile($csig_checksum_file)
    {
        ...
        return $this->verifyCsigChecksumList($signed_checksum_list, $working_directory);
    }
}
